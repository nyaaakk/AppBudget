...
{
    public partial class frmGraphCharge : Form
    {
        public string name = "Series1";
        public frmGraphCharge()
        {
            InitializeComponent();
        }

        private void frmGraphCharge_Load(object sender, EventArgs e)
        {
        ...
        private void btnAjouter_Click(object sender, EventArgs e)
        {
            ...
            max = database1DataSet.Charge.Count;
            int idLib = int.Parse(cbxLib.SelectedValue.ToString());
            for (int i = 0; i < max; i++)
            {
                if (int.Parse(database1DataSet.Charge.Rows[i]["idCharge"].ToString()) == idLib)
                {
                    if (idT1 == int.Parse(database1DataSet.Charge.Rows[i]["idDate"].ToString()))
                    {
                        valT1 = float.Parse(database1DataSet.Charge.Rows[i]["valCharge"].ToString());
                    }
                    else if (idT2 == int.Parse(database1DataSet.Charge.Rows[i]["idDate"].ToString()))
                    {
                        valT2 = float.Parse(database1DataSet.Charge.Rows[i]["valCharge"].ToString());
                    }
                    else if (idT3 == int.Parse(database1DataSet.Charge.Rows[i]["idDate"].ToString()))
                    {
                        valT3 = float.Parse(database1DataSet.Charge.Rows[i]["valCharge"].ToString());
                    }
                    else if (idT4 == int.Parse(database1DataSet.Charge.Rows[i]["idDate"].ToString()))
                    {
                        valT4 = float.Parse(database1DataSet.Charge.Rows[i]["valCharge"].ToString());
                    }
                }
            }
